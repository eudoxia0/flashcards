...
impl Card {
    ...
    pub fn html_front(&self, port: u16, collection_dir: &Path) -> Fallible<Markup> {
        self.content.html_front(port, &self.file_path, collection_dir)
    }

    pub fn html_back(&self, port: u16, collection_dir: &Path) -> Fallible<Markup> {
        self.content.html_back(port, &self.file_path, collection_dir)
    }
}
...
impl CardContent {
    ...
    pub fn html_front(&self, port: u16, card_file_path: &Path, collection_dir: &Path) -> Fallible<Markup> {
        let html = match self {
            CardContent::Basic { question, .. } => {
                html! {
                    (PreEscaped(markdown_to_html(question, port, card_file_path, collection_dir)))
                }
            }
            CardContent::Cloze { text, start, end } => {
                ...
                let text: String = markdown_to_html(&text, port, card_file_path, collection_dir);
                let text: String =
    ...
    pub fn html_back(&self, port: u16, card_file_path: &Path, collection_dir: &Path) -> Fallible<Markup> {
        let html = match self {
            CardContent::Basic { answer, .. } => {
                html! {
                    (PreEscaped(markdown_to_html(answer, port, card_file_path, collection_dir)))
                }
            }
            CardContent::Cloze { text, start, end } => {
                let mut text_bytes: Vec<u8> = text.as_bytes().to_owned();
                ...
                let deleted_text: String = markdown_to_html_inline(&deleted_text, port, card_file_path, collection_dir);
                text_bytes.splice(*start..*end + 1, CLOZE_TAG_BYTES.iter().copied());
                let text: String = String::from_utf8(text_bytes)?;
                let text = markdown_to_html(&text, port, card_file_path, collection_dir);
