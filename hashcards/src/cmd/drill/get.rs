...
fn render_card(card: &Card, reveal: bool, port: u16, collection_dir: &std::path::Path) -> Fallible<Markup> {
    let html = match card.card_type() {
        CardType::Basic => {
            if reveal {
                html! {
                    div .question .rich-text {
                        (card.html_front(port, collection_dir)?)
                    }
                    div .answer .rich-text {
                        (card.html_back(port, collection_dir)?)
                    }
                }
            } else {
                html! {
